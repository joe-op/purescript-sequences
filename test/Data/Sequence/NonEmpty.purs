...
nonEmptySequenceTests = do
  ...
  quickCheck
    $ \f z (ArbNESeq xs) ->
        let
          types = Tuple (f :: Int -> Int -> Int) (z :: Int)
        in
          foldr f z xs == foldr f z (arr xs :: Array Int)
  quickCheck
    $ \f z (ArbNESeq xs) ->
        let
          types = Tuple (f :: Int -> Int -> Int) (z :: Int)
        in
          foldl f z xs == foldl f z (arr xs :: Array Int)
  quickCheck
    $ \(ArbNESeq xs) ->
  ...
  quickCheck
    $ \idx (ArbNESeq seq) ->
        ...
        in
          S.last (fst split) == NonEmpty.index (idx' - 1) seq
            && S.head (snd split)
            == NonEmpty.index idx' seq
            <?> ("seq: " <> show seq <> ", idx':" <> show idx')
  log "Test that adjust is safe"
  quickCheck
    $ \(ArbNESeq seq) ->
        let
          f n = NonEmpty.adjust identity n (seq :: NonEmpty.Seq Int)
        in
          f (-1) == f (NonEmpty.length seq)
  log "Test that index is safe"
  quickCheck
    $ \(ArbNESeq seq) ->
        let
          f n = NonEmpty.index n (seq :: NonEmpty.Seq Int)
        in
          f (-1) == Nothing && f (NonEmpty.length seq) == Nothing
  log "Test inBounds"
  quickCheck
    $ \(ArbNESeq seq) ->
        let
        ...
        in
          NonEmpty.inBounds lowerBound seq && NonEmpty.inBounds upperBound seq
  ...
  quickCheck
    $ \(ArbNESeq seq) idx ->
        let
          seq' = const 0 <$> (seq :: NonEmpty.Seq Unit)

          idx' = integerBetween 0 (NonEmpty.length seq') idx

          result = sum (NonEmpty.adjust (_ + 1) idx' seq')
        in
          result == 1 <?> "seq': " <> show seq' <> ", result: " <> show result
  log "Test take"
  quickCheck
    $ \(ArbNESeq seq) n ->
        let
          result = S.length (NonEmpty.take n (seq :: NonEmpty.Seq Int))
        in
          0 <= result && result <= abs n
            <?> err
                [ "n = " <> show n
                , "seq = " <> show seq
                , "result = " <> show result
  ...
  quickCheck
    $ \(ArbNESeq seq) n ->
        let
          dropped = NonEmpty.length (seq :: NonEmpty.Seq Int) - S.length (NonEmpty.drop n seq)
        in
          0 <= dropped && dropped <= abs n
            <?> err
                [ "n = " <> show n
                , "seq = " <> show seq
  ...
  let
    oneToFive = unfoldr1 step1 1 :: NonEmpty.Seq Int
